...
/**
 * Scrape profile data from Apify
...
async function scrapeWithApify(username, platform) {
  ...
  if (!apiKey) {
    throw new Error('APIFY_API_KEY nao configurada');
  }

  const actorId = APIFY_CONFIG.actors[platform] || APIFY_CONFIG.actors.instagram;
  ...
  try {
    const runResponse = await axios.post(
      ...
      input,
      {
        ...
        params: { token: apiKey, timeout: APIFY_CONFIG.requestTimeout, memory: APIFY_CONFIG.memory },
        timeout: APIFY_CONFIG.timeout
      }
    );

    if (!runResponse.data || runResponse.data.length === 0) {
      throw new Error('Apify sem dados (perfil privado?)');
    }
    ...
    if (err.response) {
      ...
      if (status === 401) throw new Error('APIFY_API_KEY invalida');
      if (status === 402) throw new Error('Sem creditos Apify');
      if (status === 404) throw new Error('Actor nao encontrado: ' + actorId);
      throw new Error('Apify HTTP ' + status);
    }
...
/**
 * POST /api/social/analyze-profile
 * Analyzes a social media profile
 */
 ...
 * Compares multiple social media profiles
 */
router.post('/compare', async (req, res) => {
  const { profiles } = req.body;

  if (!Array.isArray(profiles) || profiles.length < 2) {
    return res.status(400).json({ error: 'Envie ao menos 2 perfis.' });
  }

  const profileList = profiles.map(p => 
    `@${(p.username || '').replace(/^@/, '')} (${p.platform || 'instagram'})`
  ).join('\n');
  ...
  try {
    const comparison = await askJson(
      userMessage, 
      'Analista de redes sociais. Retorne APENAS JSON em portugues do Brasil.'
    );
    res.json({ comparison });
  } catch (err) {
...
router.get('/health', (req, res) => {
  ...
  res.json({ 
    apify: hasApify, 
    mode: hasApify ? 'scraping-real' : 'claude', 
    keyPreview: hasApify ? process.env.APIFY_API_KEY.slice(0, 12) + '...' : 'nao configurada' 
  });
});
