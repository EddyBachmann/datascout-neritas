...
const PERFIL = `Dra. Roseli Perfoll - Ginecologista especializada em longevidade feminina, Balneario Camboriu SC. 
Publico-alvo: mulheres 45-70 anos. 
Tom: acolhedor, cientifico e empoderador. 
CFM: sem promessas de cura, usar "pode contribuir", "evidencias sugerem", "estudos indicam".`;

const CONTENT_SYSTEM_PROMPTS = {
  calendar: 'Voce e estrategista de conteudo medico. Retorne APENAS JSON valido em portugues do Brasil.',
  caption: 'Voce e copywriter especializado em marketing medico. Retorne APENAS JSON valido em portugues do Brasil.',
  carousel: 'Voce e criador de conteudo medico. Retorne APENAS JSON valido em portugues do Brasil.'
};

/**
 * POST /api/content/calendar
 * Gera calendario editorial de Instagram
 */
...
Gere entre 12 e 16 posts para o mes.`;
  ...
  try {
    const result = await askJson(userMessage, CONTENT_SYSTEM_PROMPTS.calendar);
    res.json(result);
  } catch (err) {
 ...
 * Gera legenda otimizada para redes sociais
 */
...
Retorne JSON: { "caption": "legenda completa com emojis e hashtags", "meta": { "tema": "...", "formato": "...", "plataforma": "..." } }`;
  ...
  try {
    const result = await askJson(userMessage, CONTENT_SYSTEM_PROMPTS.caption);
 ...
 * Expande tema em carrossel de Instagram
 */
...
Slide 1: capa impactante. Ultimos slides: CTA e contato.`;
  ...
  try {
    const result = await askJson(userMessage, CONTENT_SYSTEM_PROMPTS.carousel);
